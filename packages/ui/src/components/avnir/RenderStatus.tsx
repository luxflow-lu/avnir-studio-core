...
const statusConfig: Record<RenderStatus, {
  label: string;
  color: string;
...
export const RenderStatus = React.forwardRef<HTMLDivElement, RenderStatusProps>(
  ({ className, job, onCancel, onRetry, onDownload, showDetails = true, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cx("bg-[var(--surface)] rounded-[var(--radius-lg)] p-4 border border-white/5", className)}
        {...props}
      >
            ...
            <div className="space-y-2">
              ...
              {job.estimatedTime && (
                <div className="flex justify-between text-xs text-[var(--text-muted)]">
                  <span>
                    {processingTime ? formatDuration(processingTime) : "0:00"} écoulé
                  </span>
                  <span>
                    ~{formatDuration(job.estimatedTime)} restant
                  </span>
                </div>
              )}
            </div>
          ...
          {showDetails && (
            <div className="flex items-center justify-between text-xs text-[var(--text-muted)]">
              <div className="flex items-center gap-4">
                {job.startTime && (
                  <span>Démarré: {job.startTime.toLocaleTimeString()}</span>
                )}
                {job.endTime && (
                  <span>Terminé: {job.endTime.toLocaleTimeString()}</span>
                )}
              </div>
