...
export type NewsletterProps = { title?: string; subtitle?: string; onSubmit?: (email: string) => void; className?: string } & React.HTMLAttributes<HTMLElement>;

export const Newsletter = React.forwardRef<HTMLElement, NewsletterProps>(({ title, subtitle, onSubmit, className, ...props }, ref) => {
    ...
    <section ref={ref} className={cx("w-full mx-auto px-4 md:px-6 py-16 md:py-24", className)} {...props}>
      <div className="mx-auto max-w-2xl text-center">
        ...
        <form onSubmit={submit} className="mt-6 flex gap-3 justify-center">
          ...
          />
          <button type="submit" className="h-10 rounded-[var(--radius)] bg-[var(--brand)] text-[var(--brand-on)] px-4 font-medium">S'inscrire</button>
        </form>
      </div>
    </section>
  );
