...
export const Price = React.forwardRef<HTMLDivElement, PriceProps>(
  ({ className, amount, currency = "€", originalAmount, size = "md", showCurrency = true, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cx("flex items-baseline gap-2", className)} {...props}>
        <div className="flex items-baseline">
          {showCurrency && (
            <span className={cx("text-[var(--text-muted)]", sizeClasses.currency)}>
              {currency}
            </span>
          )}
          <span className={cx("font-bold text-white", sizeClasses.price)}>
            {amount.toFixed(2)}
          </span>
        </div>
        {hasDiscount && (
          <span className={cx("line-through text-[var(--text-muted)]", sizeClasses.original)}>
            {showCurrency && currency}{originalAmount.toFixed(2)}
          </span>
        )}
      </div>
    );
