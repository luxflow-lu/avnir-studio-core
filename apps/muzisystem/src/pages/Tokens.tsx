import React from "react";
import { Button } from "@avnir/ui";

export const Tokens: React.FC = () => {
  const colorTokens = [
    { name: "Primary", var: "--primary", value: "92 185 242", hex: "#5CB9F2" },
    { name: "On Primary", var: "--on-primary", value: "14 17 22", hex: "#0E1116" },
    { name: "Background", var: "--bg", value: "11 11 13", hex: "#0B0B0D" },
    { name: "Surface", var: "--surface", value: "20 19 23", hex: "#141317" },
    { name: "Border", var: "--border", value: "38 39 43", hex: "#26272B" },
    { name: "On Background", var: "--on-bg", value: "197 204 214", hex: "#C5CCD6" },
    { name: "On Surface", var: "--on-surface", value: "244 244 244", hex: "#F4F4F4" }
  ];

  const brandTokens = [
    { name: "AVNIR", primary: "#EDEDED", description: "Gris clair élégant" },
    { name: "MUZIDEV", primary: "#5CB9F2", description: "Bleu formation" },
    { name: "MUZIPICS", primary: "#FF2D55", description: "Rouge passion" },
    { name: "MUZIMERCH", primary: "#FF9D00", description: "Orange commerce" },
    { name: "MUZIBASE", primary: "#2FAD66", description: "Vert données" },
    { name: "MUZIMANAGER", primary: "#9802EB", description: "Violet gestion" },
    { name: "PROMOZIC", primary: "#FFD700", description: "Jaune promotion" }
  ];

  const badgeTokens = [
    { name: "Artist", var: "--badge-artist", color: "#8A5CF6", type: "Créatif" },
    { name: "Studio", var: "--badge-studio", color: "#2FAD66", type: "Production" },
    { name: "Beatmaker", var: "--badge-beatmaker", color: "#4FA4FF", type: "Technique" },
    { name: "Draft", var: "--badge-draft", color: "#FFD54A", type: "Statut" },
    { name: "Producer", var: "--badge-producer", color: "#FF9D00", type: "Business" },
    { name: "Archived", var: "--badge-archived", color: "#7A7D87", type: "Système" }
  ];

  const spacingTokens = [
    { name: "space-1", value: "0.25rem", pixels: "4px", usage: "Micro-espacements" },
    { name: "space-2", value: "0.5rem", pixels: "8px", usage: "Petits espacements" },
    { name: "space-3", value: "0.75rem", pixels: "12px", usage: "Espacements de base" },
    { name: "space-4", value: "1rem", pixels: "16px", usage: "Espacements standards" },
    { name: "space-5", value: "1.25rem", pixels: "20px", usage: "Espacements moyens" },
    { name: "space-6", value: "1.5rem", pixels: "24px", usage: "Grands espacements" },
    { name: "space-8", value: "2rem", pixels: "32px", usage: "Très grands espacements" },
    { name: "space-10", value: "2.5rem", pixels: "40px", usage: "Espacements de section" },
    { name: "space-12", value: "3rem", pixels: "48px", usage: "Sections standards" },
    { name: "space-16", value: "4rem", pixels: "64px", usage: "Sections larges" }
  ];

  return (
    <div className="min-h-screen bg-background text-foreground">
      
      {/* === HEADER === */}
      <header className="section-tight border-b border-border bg-surface">
        <div className="container">
          <div className="flex items-center justify-between">
            <div className="cluster">
              <a href="#home" className="cluster">
                <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center">
                  <span className="text-primary-foreground font-bold text-lg">M</span>
                </div>
                <span className="text-xl font-bold text-foreground">MUZISYSTEM</span>
              </a>
            </div>
            
            <nav className="hidden md:flex cluster">
              <a href="#components" className="text-foreground hover:text-primary transition-colors">Composants</a>
              <a href="#tokens" className="text-primary font-medium">Design Tokens</a>
              <a href="#guidelines" className="text-foreground hover:text-primary transition-colors">Guidelines</a>
            </nav>
            
            <div className="cluster">
              <Button variant="outline">Documentation</Button>
              <Button variant="solid">CSS Variables</Button>
            </div>
          </div>
        </div>
      </header>

      {/* === HERO === */}
      <section className="section">
        <div className="container">
          <div className="text-center stack">
            <h1 className="text-6xl font-bold text-foreground">
              Design Tokens
            </h1>
            <p className="text-xl text-foreground/80 max-w-4xl mx-auto leading-relaxed">
              Variables CSS centralisées pour maintenir la cohérence visuelle 
              à travers tous les satellites AVNIR Studio. Couleurs, espacements, 
              typographie et plus encore.
            </p>
          </div>
        </div>
      </section>

      {/* === COLOR TOKENS === */}
      <section className="section bg-surface">
        <div className="container">
          <div className="stack-lg">
            <h2 className="text-4xl font-bold text-foreground">Couleurs Système</h2>
            
            <div className="grid-3">
              {colorTokens.map((token, index) => (
                <div key={index} className="card">
                  <div className="stack">
                    <div className="cluster">
                      <div 
                        className="w-12 h-12 rounded-lg border border-border"
                        style={{ backgroundColor: token.hex }}
                      ></div>
                      <div className="stack-sm">
                        <h3 className="font-semibold text-foreground">{token.name}</h3>
                        <code className="text-sm text-foreground/70">{token.var}</code>
                      </div>
                    </div>
                    <div className="stack-sm">
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">RGB:</span>
                        <code className="text-sm font-mono">{token.value}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">HEX:</span>
                        <code className="text-sm font-mono">{token.hex}</code>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* === BRAND TOKENS === */}
      <section className="section">
        <div className="container">
          <div className="stack-lg">
            <h2 className="text-3xl font-bold text-foreground">Couleurs de Marque</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {brandTokens.map((brand, index) => (
                <div key={index} className="card">
                  <div className="stack">
                    <div className="cluster">
                      <div 
                        className="w-12 h-12 rounded-lg"
                        style={{ backgroundColor: brand.primary }}
                      ></div>
                      <div className="stack-sm">
                        <h3 className="font-semibold text-foreground">{brand.name}</h3>
                        <p className="text-sm text-foreground/70">{brand.description}</p>
                      </div>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-foreground/70">Primary:</span>
                      <code className="text-sm font-mono">{brand.primary}</code>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* === BADGE TOKENS === */}
      <section className="section bg-surface">
        <div className="container">
          <div className="stack-lg">
            <h2 className="text-3xl font-bold text-foreground">Badges Typés</h2>
            
            <div className="grid-3">
              {badgeTokens.map((badge, index) => (
                <div key={index} className="card">
                  <div className="stack">
                    <div className="cluster">
                      <div 
                        className="w-12 h-12 rounded-lg flex items-center justify-center text-white font-medium"
                        style={{ backgroundColor: badge.color }}
                      >
                        {badge.name.charAt(0)}
                      </div>
                      <div className="stack-sm">
                        <h3 className="font-semibold text-foreground">{badge.name}</h3>
                        <span className="text-sm text-foreground/70">{badge.type}</span>
                      </div>
                    </div>
                    <div className="stack-sm">
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">Variable:</span>
                        <code className="text-sm font-mono">{badge.var}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">Couleur:</span>
                        <code className="text-sm font-mono">{badge.color}</code>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
            </div>

            <div className="card">
              <div className="stack">
                <h3 className="text-lg font-semibold text-foreground">Aperçu des badges</h3>
                <div className="cluster">
                  <span className="badge badge-artist">Artist</span>
                  <span className="badge badge-studio">Studio</span>
                  <span className="badge badge-beatmaker">Beatmaker</span>
                  <span className="badge badge-draft">Draft</span>
                  <span className="badge badge-producer">Producer</span>
                  <span className="badge badge-archived">Archived</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* === SPACING TOKENS === */}
      <section className="section">
        <div className="container">
          <div className="stack-lg">
            <h2 className="text-3xl font-bold text-foreground">Espacements</h2>
            
            <div className="grid-3">
              {spacingTokens.map((spacing, index) => (
                <div key={index} className="card">
                  <div className="stack">
                    <div className="cluster">
                      <div 
                        className="bg-primary rounded"
                        style={{ width: spacing.value, height: spacing.value, minWidth: '16px', minHeight: '16px' }}
                      ></div>
                      <div className="stack-sm">
                        <h3 className="font-semibold text-foreground">{spacing.name}</h3>
                        <p className="text-sm text-foreground/70">{spacing.usage}</p>
                      </div>
                    </div>
                    <div className="stack-sm">
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">REM:</span>
                        <code className="text-sm font-mono">{spacing.value}</code>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-foreground/70">Pixels:</span>
                        <code className="text-sm font-mono">{spacing.pixels}</code>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* === USAGE SECTION === */}
      <section className="section bg-surface">
        <div className="container">
          <div className="stack-lg">
            <h2 className="text-3xl font-bold text-foreground">Utilisation</h2>
            
            <div className="grid-2">
              <div className="card">
                <div className="stack">
                  <h3 className="text-xl font-semibold text-foreground">CSS Variables</h3>
                  <pre className="bg-bg p-4 rounded-lg text-sm overflow-x-auto">
                    <code>{`/* Utilisation des tokens */
.my-component {
  background: rgb(var(--primary));
  color: rgb(var(--on-primary));
  border: 1px solid rgb(var(--border));
  padding: var(--space-6);
}

/* Badge typé */
.badge-custom {
  background: var(--badge-artist);
  color: white;
}`}</code>
                  </pre>
                </div>
              </div>

              <div className="card">
                <div className="stack">
                  <h3 className="text-xl font-semibold text-foreground">Tailwind Classes</h3>
                  <pre className="bg-bg p-4 rounded-lg text-sm overflow-x-auto">
                    <code>{`<!-- Classes Tailwind générées -->
<div class="bg-primary text-on-primary">
  Primary background
</div>

<div class="bg-surface text-foreground border-border">
  Surface avec bordure
</div>

<span class="badge badge-artist">
  Badge typé
</span>`}</code>
                  </pre>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* === FOOTER === */}
      <footer className="section-tight border-t border-border">
        <div className="container">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-foreground/70">© 2025 MUZISYSTEM by AVNIR Studio</p>
            <div className="cluster">
              <a href="#components" className="text-foreground/70 hover:text-primary transition-colors">Composants</a>
              <a href="#guidelines" className="text-foreground/70 hover:text-primary transition-colors">Guidelines</a>
              <a href="#" className="text-foreground/70 hover:text-primary transition-colors">GitHub</a>
            </div>
          </div>
        </div>
      </footer>
      
    </div>
  );
};
